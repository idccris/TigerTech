...
export default function AdminUsers() {
  ...
  return <main className="admin-users-page"><AdminNavbar />
    ...
    <section className="users-list">
      {users.map((user) => <article key={user.username}><div className="user-account"><strong>{user.display_name || user.username}</strong><span>@{user.username}</span></div><em className={`user-role ${user.role}`}>{user.role === "admin" ? "Administrador" : "Operador"}</em><small className={`user-status ${user.active ? "active" : "inactive"}`}>{user.active ? "Acesso ativo" : "Desativado"}</small><button className="user-toggle" onClick={() => toggle(user)} aria-label={`${user.active ? "Desativar" : "Ativar"} acesso de ${user.display_name || user.username}`}>{user.active ? "Desativar" : "Ativar"}</button></article>)}
    </section>
