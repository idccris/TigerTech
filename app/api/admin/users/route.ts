...
export async function PATCH(req: Request) {
  ...
  if (body.action === "reset-password") {
    ...
    if (newPassword.length < 12 || !/[a-z]/i.test(newPassword) || !/\d/.test(newPassword))
      return Response.json({ error: "Use pelo menos 12 caracteres, incluindo letras e números." }, { status: 400 });
    const currentRows = await sql()`SELECT password_hash FROM admins WHERE username=${current} AND active=true LIMIT 1`;
    if (!currentRows[0] || !(await bcrypt.compare(adminPassword, String(currentRows[0].password_hash))))
      return Response.json({ error: "Sua senha de administrador está incorreta." }, { status: 403 });
    const targetRows = await sql()`SELECT role,active FROM admins WHERE username=${username} LIMIT 1`;
    if (!targetRows[0]) return Response.json({ error: "Usuário não encontrado." }, { status: 404 });
    if (targetRows[0].role === "admin" && targetRows[0].active === false) {
      const count = await sql()`SELECT COUNT(*)::int count FROM admins WHERE role='admin' AND active=true AND username<>${username}`;
      if (Number(count[0]?.count) >= 2)
        return Response.json({ error: "O limite é de 2 administradores ativos." }, { status: 409 });
    }
    const hash = await bcrypt.hash(newPassword, 12);
    await sql()`UPDATE admins SET password_hash=${hash},active=true,updated_at=NOW() WHERE username=${username}`;
